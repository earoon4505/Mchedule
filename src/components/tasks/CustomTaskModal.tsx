...
import { Plus, Minus, AlertCircle, Calendar, Clock, RotateCcw } from 'lucide-react';
...
export const CustomTaskModal: React.FC<CustomTaskModalProps> = ({
  ...
  const [resetMode, setResetMode] = useState<'dayOfWeek' | 'date'>('dayOfWeek'); // 요일 또는 날짜 선택
  const [isDaily, setIsDaily] = useState<boolean>(false);
  const [selectedDays, setSelectedDays] = useState<number[]>([4]); // 기본 목요일 (최대 6개)
  const [selectedDayOfMonth, setSelectedDayOfMonth] = useState<number>(1); // 1 ~ 31 (기본 1일)
  const [resetHour, setResetHour] = useState<number>(0); // 0 ~ 23
  const [resetMinute, setResetMinute] = useState<number>(0); // 0 ~ 55, 5단위
  ...
  return (
    <div 
      id="custom-task-modal-backdrop"
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200"
    >
      <div 
        id="custom-task-modal-panel"
      ...
      >
        <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800 mb-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-base text-slate-900 dark:text-white">커스텀 등록</h3>
          </div>
        </div>
