...
export const StandalonePiPView: React.FC = () => {
  ...
  const renderFixedCommonContents = () => {
    ...
    // 1. 세로 모드일 때: 캐릭터 카드 맨 위에 가로 방향 일렬로 정렬 (너비: 240px 맞춤, 5개 이상일 경우 적응형 크기로 짤림 방지)
              ...
              >
                <div className="relative flex items-center justify-center pointer-events-none select-none">
                  <MapleIcon
                    ...
                    className={`${isMany ? 'w-6 h-6' : 'w-7 h-7'} object-contain rounded-lg transition-all select-none pointer-events-none ${
                      isDone ? 'grayscale-0 opacity-100 scale-105' : 'grayscale-50 opacity-60'
                    }`}
                  />
                  {isDone && (
                    <div className={`absolute -bottom-1 -right-1 ${isMany ? 'w-3 h-3 text-[7px]' : 'w-3.5 h-3.5 text-[8px]'} rounded-full text-white flex items-center justify-center shadow-xs font-bold pointer-events-none ${
    ...
    return (
      <div 
        ...
        className="h-[104px] min-h-[104px] max-h-[104px] p-1 flex-shrink-0 bg-transparent border-0 shadow-none box-border flex items-center justify-center cursor-grab active:cursor-grabbing select-none my-1"
      >
        {isOdd ? (
